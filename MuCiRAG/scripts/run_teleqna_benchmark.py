...
DEBUG_RECORD_KEY = "_debug_record"

from MuCiRAG.src.evaluation.teleqna import (
    compact_retrieval_trace,
    parse_record,
    score_multiple_choice,
)
from MuCiRAG.src.evaluation.tracking import start_experiment_tracker
from MuCiRAG.src.rag.corpus import HYBRID_CANDIDATE_MULTIPLIER, HYBRID_RRF_K
from MuCiRAG.src.rag.service import MuCiRAGService
from MuCiRAG.src.settings import load_settings


def resolve_MuCiRAG_path(path: Path) -> Path:
    """Resolve CLI paths from MuCiRAG/, accepting an accidental leading prefix."""
    if path.is_absolute():
        return path
    if path.parts and path.parts[0] == "MuCiRAG":
    ...
    if config["anchor_strategy"] == "hierarchical":
        ...
        if not artifact_path.is_file():
            raise FileNotFoundError(
                ...
                "`cd MuCiRAG && uv run python scripts/embed_anchor_hierarchy.py` first."
            )
...
def debug_output_path(output_path: Path) -> Path:
    """Name the companion checkpoint containing full human-readable context."""
    return output_path.with_name(f"{output_path.stem}_debug.json")


def _format_debug_records(records: list[dict[str, Any]]) -> str:
    """Serialize debug records as a readable JSON array with a stable closing tail."""
    rendered = ",\n".join(json.dumps(record, ensure_ascii=False, indent=2) for record in records)
    return f"[\n{rendered}\n]\n"


def initialize_debug_output(path: Path) -> int:
    """Create or normalize a valid JSON debug array before appending checkpoints."""
    if path.exists():
        payload = json.loads(path.read_text(encoding="utf-8"))
        if not isinstance(payload, list) or not all(isinstance(record, dict) for record in payload):
            raise ValueError(f"Debug output must be a JSON array of records: {path}")
        records = payload
    else:
        records = []
    path.write_text(_format_debug_records(records), encoding="utf-8")
    return len(records)


def append_debug_record(debug_file, record: dict[str, Any], has_existing_records: bool) -> None:
    """Append one record while keeping the companion file valid, formatted JSON."""
    debug_file.seek(-2, 2)  # Remove the trailing `]\n` before adding an array element.
    debug_file.truncate()
    if has_existing_records:
        debug_file.write(b",\n")
    debug_file.write(json.dumps(record, ensure_ascii=False, indent=2).encode("utf-8"))
    debug_file.write(b"\n]\n")
    debug_file.flush()
...
def debug_record(
    *,
    ...
    """Render the exact ordered context blocks supplied to the answer model."""
    seeds = [hit for hit in retrievals if hit.origin != "citation"]
    citations = [hit for hit in retrievals if hit.origin == "citation"]
    chunks = []
    for position, hit in enumerate([*seeds, *citations], start=1):
        ...
        chunks.append(
            {
                ...
                "citation_depth": hit.citation_depth if hit.origin == "citation" else None,
                "parent_chunk_id": hit.parent_chunk_id if hit.origin == "citation" else None,
                "content": MuCiRAGService._format_context(hit),
            }
        )
    return {
        ...
        "ground_truth": {"expected_option": expected_option},
        "prediction": {
...
def evaluate_record(service: MuCiRAGService, question_id: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    row[DEBUG_RECORD_KEY] = debug_record(
        question_id=question_id,
...
def parse_args(settings: Any) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Checkpoint JSONL, resolved from MuCiRAG/. "
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Existing benchmark JSONL, resolved from MuCiRAG/. "
    ...
    args.dataset = resolve_MuCiRAG_path(args.dataset)
    if args.output is not None:
        args.output = resolve_MuCiRAG_path(args.output)
    if args.compare_to is not None:
        args.compare_to = resolve_MuCiRAG_path(args.compare_to)
...
def main() -> None:
    ...
    debug_path = debug_output_path(args.output)
    if args.overwrite:
        args.output.unlink(missing_ok=True)
        debug_path.unlink(missing_ok=True)
    debug_record_count = initialize_debug_output(debug_path)
    ...
    def write_row(output_file, debug_file, row: dict[str, Any]) -> None:
        nonlocal correct, evaluated, debug_record_count
        debug_row = row.pop(DEBUG_RECORD_KEY)
        question_id = row["question_id"]
        output_file.write(json.dumps(row, ensure_ascii=False) + "\n")
        output_file.flush()
        append_debug_record(debug_file, debug_row, has_existing_records=debug_record_count > 0)
    ...
    try:
        with (
            ...
            debug_path.open("r+b") as debug_file,
        ):
            if args.workers == 1:
                service = MuCiRAGService(settings=settings)
                for question_id, payload in pending_records:
                    write_row(output_file, debug_file, evaluate_record(service, question_id, payload))
            else:
                ...
                with ThreadPoolExecutor(max_workers=args.workers) as executor:
                    ...
                    for row in executor.map(evaluate_in_worker, pending_records):
                        write_row(output_file, debug_file, row)
    ...
    print(
        f"Done: {len(ordered_ids)}/{len(ordered_ids)} | accuracy={accuracy:.1%} | "
        f"results={args.output} | debug={debug_path}"
    )
